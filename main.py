...
async def tournament_add_ticket(
    ...
    try:
        linked1 = await arun(db.get_linked_account, player1.id) or {"minecraft_name": "ismeretlen"}
        linked2 = await arun(db.get_linked_account, player2.id) or {"minecraft_name": "ismeretlen"}
    except Exception as exc:
        await interaction.followup.send(
    ...
    try:
        ...
        await interaction.followup.send(
            f"DB hiba a match létrehozásakor: `{exc}`", ephemeral=True
        )
        return
