...
class NeonTiersBot(commands.Bot):
    ...
    async def _sync_guild_commands(self) -> bool:
        """Sync slash commands. If GUILD_ID is 0, sync globally instead."""
        global _synced
...
async def dbtest_cmd(interaction: discord.Interaction) -> None:
    """End-to-end Supabase sanity check.

    Runs INSERT → SELECT → UPDATE → DELETE on the tournaments table with a
    throwaway test row and reports each step's outcome. This is the fastest
    way to figure out why tournaments aren't persisting.
    """
    await interaction.response.defer(ephemeral=True, thinking=True)

    import uuid as _uuid
    from utils import utcnow
    from database import db as _db, arun as _arun

    lines: list[str] = []
    lines.append("**🔧 Supabase diagnosztika**")
    lines.append(f"• URL: `{config.supabase_url}`")
    anon_masked = config.supabase_anon_key[:12] + "…" if config.supabase_anon_key else "(üres)"
    lines.append(f"• Anon key: `{anon_masked}`")
    lines.append(f"• Guild ID: `{config.guild_id}`")
    lines.append("")

    test_id = str(_uuid.uuid4())
    test_name = "dbtest-please-delete"

    # Step 1: INSERT
    try:
        await _arun(
            _db.create_tournament,
            name=test_name,
            end_time=utcnow(),
            queue_message_id=0,
        ...
        lines.append("")
        lines.append("**Diagnosztika:** ez általában RLS policy vagy schema hiba.")
        lines.append("Futtasd le a `supabase_setup.sql`-t a Supabase SQL Editor-ben.")
        await interaction.followup.send("\n".join(lines), ephemeral=True)
        return

    # Step 2: SELECT
    try:
        got = await _arun(_db.get_tournament, test_id)
        if got is None:
            lines.append("❌ **SELECT** visszatért `None`-al — a sor nem olvasható.")
            lines.append("Ez azt jelenti, hogy az RLS blokkolja a SELECT-et anonra.")
        else:
    ...
    # Step 3: UPDATE
    try:
        await _arun(_db.update_tournament, test_id, queue_message_id=999999)
    ...
    # Step 4: DELETE
    try:
        await _arun(_db.delete_tournament, test_id)
        lines.append("✅ **DELETE** sikeres")
    except Exception as exc:
        lines.append(f"❌ **DELETE** hiba: `{type(exc).__name__}: {exc}`")

    # Step 5: List active (verify the table is queryable end-to-end)
    try:
        active = await _arun(_db.list_active_tournaments)
        lines.append(f"✅ **list_active_tournaments**: {len(active)} aktív tournament")
    except Exception as exc:
        lines.append(f"❌ **list_active_tournaments** hiba: `{type(exc).__name__}: {exc}`")

    lines.append("")
    lines.append("**Összegzés:** ha bármelyik lépés ❌, a Supabase nincs rendben.")
    await interaction.followup.send("\n".join(lines), ephemeral=True)
