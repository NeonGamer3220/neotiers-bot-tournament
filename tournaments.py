...
class TournamentCog(commands.Cog):
    ...
    async def tournamentqueue(
        ...
        try:
            ...
            # Catch EVERYTHING — supabase-py raises APIError (not RuntimeError)
            # for RLS / schema / network failures. Without this the exception
            # silently kills the task and Discord just shows "thinking…".
            log.exception("create_tournament failed (type=%s)", type(exc).__name__)
            await interaction.followup.send(
                f"❌ Hiba a Tournament létrehozásakor.\n"
                f"**Típus:** `{type(exc).__name__}`\n"
                f"**Üzenet:** `{exc}`\n"
                f"**Tournament ID:** `{tournament_id}`\n\n"
                f"Ellenőrizd a Railway logokat és a Supabase RLS policy-ket.",
                ephemeral=True,
            )
            return

        # VERIFY the row actually landed — supabase-py sometimes returns
        # success without persisting if RLS blocks silently.
        try:
            verify = await arun(db.get_tournament, tournament_id)
        except Exception as exc:
            log.exception("verify get_tournament failed (type=%s)", type(exc).__name__)
            verify = None
        if verify is None:
            log.error(
                "VERIFY FAILED: tournament %s not readable after insert. "
                "This means RLS is blocking SELECT, or the row was rejected.",
                tournament_id,
            )
            await interaction.followup.send(
                f"❌ A Tournament sort beszúrtuk, de nem tudjuk visszolvasni.\n"
                f"**Tournament ID:** `{tournament_id}`\n\n"
                f"Ez azt jelenti, hogy az RLS policy blokkolja a SELECT-et, "
                f"vagy a sor nem került be az adatbázisba.\n"
                f"Futtasd a `/dbtest` parancsot a diagnózishoz.",
                ephemeral=True,
            )
            return
        log.info("VERIFY OK: tournament %s readable after insert.", tournament_id)
    ...
    async def _start_round(
        ...
        async with lock:
            ...
            # Per-tournament config (fall back to global config if not set).
            tournament_ticket_category_id = int(tournament.get("ticket_category_id") or 0) or config.ticket_category_id
            tournament_regulator_role_id = int(tournament.get("regulator_role_id") or 0) or config.regulator_role_id

            category = (
                guild.get_channel(tournament_ticket_category_id)
                if tournament_ticket_category_id
                else None
            )
            if tournament_ticket_category_id and not isinstance(category, discord.CategoryChannel):
                log.warning(
                    "ticket_category_id %s is not a category — tickets will be top-level.",
                    tournament_ticket_category_id,
                )
                category = None
            ...
            for idx, (p1, p2) in enumerate(pairs, start=1):
                ...
                if tournament_regulator_role_id:
                    reg_role = guild.get_role(tournament_regulator_role_id)
                ...
                try:
                    ...
                    log.info(
                        "ticket %s created for match %s (%s vs %s) in tournament %s",
                        ticket_channel.id, match_id, p1_mc, p2_mc, tournament_id,
                    )
                except discord.HTTPException as exc:
                    log.warning("ticket message send failed (channel=%s): %s",
                                ticket_channel.id, exc)
