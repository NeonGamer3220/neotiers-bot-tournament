...
async def handle_join(interaction: discord.Interaction, tournament_id: str) -> None:
    ...
    try:
        ...
        await interaction.followup.send(
            f"DB hiba a Tournament lekérésekor: `{exc}`", ephemeral=True
        )
        return
    if not tournament:
        await interaction.followup.send(
    ...
    try:
        ...
        await interaction.followup.send(
            f"DB hiba a linked_accounts lekérésekor: `{exc}`", ephemeral=True
        )
        return
    if not linked:
        code = generate_code()
        try:
            ...
            await interaction.followup.send(
                f"Nem sikerült létrehozni a linkelési kódot: `{exc}`",
                ephemeral=True,
            )
            return
        try:
            ...
            await interaction.followup.send(
                ...
                "majd kattints újra a Belépés gombra.",
                ephemeral=True,
            )
            return
        await interaction.followup.send(
            "Nincs összekapcsolt fiókod — elküldtem a titkos kódot DM-ben. "
            "Kövesd az utasításokat, majd kattints újra a Belépés gombra.",
            ephemeral=True,
        )
        return
    ...
    try:
        added = await arun(
            db.add_player_to_tournament, tournament_id, discord_id, minecraft_name
        )
    except Exception as exc:
        await interaction.followup.send(
            f"DB hiba a csatlakozáskor: `{exc}`", ephemeral=True
        )
        return
    if not added:
        await interaction.followup.send(
            "Már csatlakoztál ehhez a Tournament-hez.", ephemeral=True
        )
        return

    await interaction.followup.send(
        f"Sikeresen csatlakoztál a **{tournament['name']}** Tournament-hez, "
...
async def handle_leave(interaction: discord.Interaction, tournament_id: str) -> None:
    ...
    try:
        ...
        await interaction.followup.send(f"DB hiba: `{exc}`", ephemeral=True)
        return
    if not removed:
        await interaction.followup.send(
            "Nem voltál regisztrálva erre a Tournament-re.", ephemeral=True
        )
        return

    await interaction.followup.send(
        f"Sikeresen kiléptél a **{tournament['name']}** Tournament-ből.",
        ephemeral=True,
    )
    await refresh_queue_embed(interaction, tournament)
...
class ResultModal(ui.Modal, title="Eredmény beírása"):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            await interaction.response.send_message(
                f"DB hiba az eredmény mentésekor: `{exc}`", ephemeral=True
            )
            return
        tournament_name = (
        ...
        results_channel = interaction.guild.get_channel(config.results_channel_id) if interaction.guild else None
