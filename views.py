...
class QueueJoinView(ui.View):
    ...
    def __init__(self, tournament_id: str) -> None:
        ...
        self.join_button.custom_id = f"join_tournament_{tournament_id}"
        self.leave_button.custom_id = f"leave_tournament_{tournament_id}"

    @ui.button(label="Csatlakozás", style=discord.ButtonStyle.success, emoji="✅")
    ...
    @ui.button(label="Kilépés", style=discord.ButtonStyle.danger, emoji="❌")
...
def build_queue_embed(tournament: dict, players: list[dict]) -> discord.Embed:
    ...
    embed.set_footer(text="Kattints a Csatlakozás gombra a jelentkezéshez.")
    return embed
...
class TicketActionView(ui.View):
    ...
    def __init__(self, match_id: str) -> None:
        ...
        self.close_button.custom_id = f"close_ticket_{match_id}"
        self.result_button.custom_id = f"result_{match_id}"

    @ui.button(label="Jegy lezárása", style=discord.ButtonStyle.secondary, emoji="🔒")
    ...
    @ui.button(label="Eredmény beírása", style=discord.ButtonStyle.primary, emoji="📝")
    async def result_button(self, interaction: discord.Interaction, button: ui.Button) -> None:
        if config.regulator_role_id not in {r.id for r in interaction.user.roles}:
            await interaction.response.send_message(
                "Ezt csak a Regulator szerepkörrel rendelkező tagok használhatják.",
                ephemeral=True,
