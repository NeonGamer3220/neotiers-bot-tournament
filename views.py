...
class FFChoiceView(ui.View):
    """Ephemeral segéd-view a Regulator számára: ki FF-elt (1, 2, vagy mindkettő)."""
    ...
    async def _finish(self, interaction: discord.Interaction, ff_choice: str):
        ...
        # Single FF -> ask for the final score first via modal.
...
class FFScoreModal(ui.Modal, title="Végső Eredmény (FF)"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        winner_id = p1_id if self.ff_choice == "p1" else p2_id
        winner_mc = p1_mc if self.ff_choice == "p1" else p2_mc
        loser_mc = p2_mc if self.ff_choice == "p1" else p1_mc

        score_val = self.score_input.value.strip() or "?"

        await arun(db.set_match_winner, match_data["id"], winner_id, None, None, True)

        text = (
            f"**{round_num}. kör eredmény**\n\n"
            f"`{winner_mc}` ff-fel jutott tovább `{loser_mc}` ellen. Végső score: {score_val}"
        )
