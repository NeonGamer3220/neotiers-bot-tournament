...
async def arun(func: "Callable[..., T]", /, *args: Any, **kwargs: Any) -> T:
    ...
    The `supabase-py` client uses `httpx` synchronously, so every DB call
    blocks the asyncio event loop. Wrapping each call in `asyncio.to_thread`
    frees the loop to process other interactions (button clicks, slash
    commands) while we wait for the HTTP response.
    """
...
class Database:
    ...
    def create_pending_code(self, discord_id: int, code: str, ttl_minutes: int) -> dict[str, Any]:
        ...
        if not resp.data:
            raise RuntimeError(
                "Supabase insert into pending_codes returned no data. "
                "This usually means Row Level Security is enabled on the "
                "table and the anon key has no INSERT policy. "
                "Add a policy like: "
                "create policy \"anon can insert pending_codes\" "
                "on public.pending_codes for insert to anon with check (true);"
            )
        return resp.data[0]
    ...
    def create_tournament(
        ...
        if not resp.data:
            raise RuntimeError(
                "Supabase insert into tournaments returned no data. "
                "This usually means Row Level Security is enabled on the "
                "tournaments table and the anon key has no INSERT policy. "
                "Add a policy like: "
                "create policy \"anon can insert tournaments\" "
                "on public.tournaments for insert to anon with check (true);"
            )
        return resp.data[0]
    ...
    def delete_tournament(self, tournament_id: str) -> None:
        """Hard-delete a tournament row. Used to roll back if the Discord
        message send fails after the row was inserted."""
    ...
    def create_match(
        ...
        if not resp.data:
            raise RuntimeError(
                "Supabase insert into matches returned no data — "
                "check RLS policies on the matches table."
            )
        return resp.data[0]
