...
async def dbtest_cmd(interaction: discord.Interaction) -> None:
    ...
    # Also test matches table.
    try:
        m = await arun(
            db.create_match, test_id, 0, 1, 2, "A", "B", 0
        )
        await arun(db.set_match_winner, m["id"], 1)
        await arun(db.delete_tournament, test_id)  # cascades
        lines.append("✅ **matches** INSERT/UPDATE/cascade OK")
    except Exception as exc:
        lines.append(f"❌ **matches** hiba: `{type(exc).__name__}: {exc}`")
        lines.append("   → A `matches` tábla hiányzik? Futtasd le a SQL setup-ot!")
