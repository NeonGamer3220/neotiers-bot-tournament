...
async def on_interaction(interaction: discord.Interaction):
    if interaction.type == discord.InteractionType.component:
        ...
        if custom_id.startswith('join_tournament_'):
            ...
            # Check linked
            linked, error = supabase.table('linked_accounts').select('minecraft_username').eq('discord_id', interaction.user.id).execute()
            if not linked[1]:
                await interaction.response.send_message("You must link your Minecraft account first.", ephemeral=True)
                return
            minecraft_username = linked[1][0]['minecraft_username']
            # Get tournament
            ...
            if any(p['discord_id'] == interaction.user.id for p in players):
                ...
                return
            new_players = players + [{'discord_id': interaction.user.id, 'minecraft_username': minecraft_username}]
...
async def start_tournament(tournament_id):
    ...
    guild = client.get_guild(tournament[1][0]['guild_id'])
    category = await guild.fetch_channel(int(os.getenv('TICKET_CATEGORY_ID')))
    for match in matches:
        channel_name = f"t-r1-{match['p1']['minecraft_username']}-{match['p2']['minecraft_username']}"
        overwrites = {
...
async def start_round(tournament_id, round_num):
    ...
    category = await guild.fetch_channel(int(os.getenv('TICKET_CATEGORY_ID')))
    for match in matches:
        channel_name = f"t-r{round_num}-{match['p1']['minecraft_username']}-{match['p2']['minecraft_username']}"
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False),
            guild.get_member(match['p1']['discord_id']): discord.PermissionOverwrite(view_channel=True),
            guild.get_member(match['p2']['discord_id']): discord.PermissionOverwrite(view_channel=True)
...
async def check_round_complete(tournament_id):
    ...
    if all_done:
        ...
        for w in winners:
            linked, error = supabase.table('linked_accounts').select('discord_id').eq('minecraft_username', w).execute()
            winners_with_discord.append({'discord_id': linked[1][0]['discord_id'], 'minecraft_username': w})
